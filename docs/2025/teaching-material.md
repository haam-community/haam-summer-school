# Teaching Material

All teaching slides from the 2025 summer school will be posted here.

The colour of each session header denotes the type of session:

- <span class="theory">Theoretical session</span>
- <span class="practical">Practical session</span>

### Day 1

<span class="theory">Introduction to HAAM, GatherTown and VMs (Thiseas C. Lamnidis)</span>

<!-- tabs:start -->
## **Recording**

<p>
<iframe width="960" height="569" src="https://www.youtube.com/embed/lEZbY8c75xs" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</p>
<!-- tabs:end -->

<span class="theory">Introduction to BASH (Thiseas C. Lamnidis)</span>

<!-- tabs:start -->
## **Recording**

<p>
<iframe width="960" height="569" src="https://www.youtube.com/embed/uPNuEeK9k3U" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
For the content of this lecture please refer to the <a href="https://www.spaam-community.org/intro-to-ancient-metagenomics-book/bare-bones-bash.html">Intro to Ancient Metagenomics Book</a>.
</p>
<!-- tabs:end -->


### Day 2

<span class="theory">Introduction to NGS data (Emily Breslin)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
  <iframe src="https://drive.google.com/file/d/1AZBJB2aFhHblhsxolxnJDrojwRSOhNrJ/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
<iframe width="960" height="569" src="https://www.youtube.com/embed/rPpTBNFTxJ8" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</p>
<!-- tabs:end -->


<span class="practical">Introduction to nf-core/eager & Interpretting nf-core/eager output (James Fellows Yates & Selina Carlhoff)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
  <iframe src="https://docs.google.com/presentation/d/e/2PACX-1vRUbGX51DUwFLJpeMi-cCeDPrbcHHkELgnWnmqt86sx0YiMW-1exn2ych1g0Kahg9Ke_BQfxlphHrna/pubembed?start=false&loop=false&delayms=3000" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
  A list of the commands used in the session can be found <a href="https://hackmd.io/@jfy133/SJ2NGWzZgl">here</a>.
</p>

## **Recording**

<p>
<iframe width="960" height="569" src="https://www.youtube.com/embed/ufa-pM8GNDU" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</p>
<!-- tabs:end -->


### Day 3
  
<span class="theory">Introduction to genetic relatedness (Şevval Aktürk)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
  <iframe src="https://drive.google.com/file/d/1oY9Kuuxwzx8fa12FOwligbj7q3-lIA5G/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->

<span class="practical">Pairwise mismatch and IBD methods (Şevval Aktürk)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/17kYQ2nXzLSqKY8od4G8k0_QYmCOdD0cz/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>

A list of the commands used in the session can be found <a href="https://drive.google.com/uc?export=download&id=1uVHrg7XnlYXba8U46zQq7r4pvh2A2Ozi">here</a>.
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<span class="theory">IBD-based Methods II (Harald Ringbauer)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1rtCmEeHE0M2rNqd0I10s3hpT99AnxBw0/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<span class="practical">HapROH (Eirini Skourtanioti)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
For the content of this lecture please refer to the  <a href="https://github.com/haam-community/summer_school_vms/blob/main/data/haproh/HAAM25_hapROH.ipynb">Jupyter Notebook used in the session</a>.
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->

### Day 4

<span class="practical">Data Curation and Unsupervised Analysis (Jazmín Ramos-Madrigal)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
Coming soon.
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->

<span class="theory">Introduction to Population Genetics (Stephan Schiffels)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1UWuVHkObSO_2tTDsLuzSlu3J7RyGFonN/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->

<span class="practical">Unsupervised Analysis (Jazmín Ramos-Madrigal)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
Coming soon.
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->

<span class="theory">Imputation (Bárbara Sousa da Mota)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1eTg9Yvf-4s3DM0kDVbnIsbF74N20aNwf/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<span class="theory">Archaeological Interpretation (John Robb)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1YwlcfmIsquN6OLVfMG9wuAvmoTZgtCTR/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


### Day 5

<span class="theory">Population Genetics II: f-stats (Leo Speidel)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/11rR5K-j7ANS2M_xuK1yp8Jm2OzWkCVF0/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<span class="practical">Admixtools2 (Leo Speidel)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1LN4TGWxX0T1-YP6WV9sh4vzwQhz9SZXU/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<span class="theory">qpAdm and IBD: Interpretation (Éadaoin Harney)</span>

<!-- tabs:start -->
## **Lecture Slides**

<p>
<iframe src="https://drive.google.com/file/d/1HzknuDP0LLGykvhCPRG10j1XocJlQYRU/preview" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</p>

## **Recording**

<p>
Coming soon.
</p> 
<!-- tabs:end -->


<style>
  .theory {
    color: #80BAE0;
    font-weight: bold;
  }

  .practical {
    color: #FD9C87;
    font-weight: bold;
  }
</style>
